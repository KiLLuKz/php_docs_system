  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#000000] text-white font-[system-ui,-apple-system,sans-serif] selection:bg-[#2997ff] selection:text-white">
      <div className="flex flex-1 flex-col md:flex-row relative">
        ...
        {/* Left Side: Beams Background (Absolute on Mobile, 60% Width on Desktop) */}
        ...
        {/* Right Side: Register Form (100% on Mobile, 40% on Desktop) */}
          ...
          <div className="w-full max-w-sm mx-auto">
            <div className="flex flex-col items-start mb-10">
              ...
              <p className="text-[15px] text-[#ffffff]/60 tracking-tight">
                ลงทะเบียนเพื่อเข้าถึงเอกสารของคุณ
              </p>
            </div>
            ...
            {success && (
              <div className="bg-[#34c759]/10 border border-[#34c759]/30 text-[#34c759] rounded-[12px] mb-6 p-4 flex text-[14px]">
                <span>{success}</span>
              </div>
            )}
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-4">
              <div>
                <input 
                  type="text" 
                  placeholder="ชื่อ-นามสกุลจริง (Full Name)" 
              ...
              <div className="pt-4">
                <button type="submit" className="w-full bg-[#0066cc] text-white hover:bg-[#0071e3] rounded-[12px] h-[50px] text-[17px] font-medium active:scale-95 transition-all duration-200" disabled={loading}>
                  {loading ? 'กำลังลงทะเบียน...' : 'ยืนยันสมัครสมาชิก'}
                </button>
              </div>
              
              <div className="text-center mt-6 pt-4 text-[14px]">
                <span className="text-[#ffffff]/60 tracking-tight">มีบัญชีอยู่แล้ว? </span>
                <Link to="/login" className="text-[#2997ff] font-medium hover:underline underline-offset-4 tracking-tight">
                  เข้าสู่ระบบเลย
                </Link>
              </div>
            </form>
